...
import { Filter, Grid, List, ChevronDown, ChevronUp, X, Leaf } from "lucide-react";
...
export default function VeganProductsPage() {
  ...
  const fetchAllData = async () => {
    try {
      ...
      // Filter only vegan products
      const allProducts = Array.isArray(productsData) ? productsData : [];
      const veganProducts = allProducts.filter(product => product.Vegan === true);
  ...
  // Get available filters based on vegan products
  const availableFilters = useMemo(() => {
    ...
    // Filter brands to only show those available in vegan products
  ...
  // Filter vegan products based on selected filters and search
      ...
      <div>
        ...
        <div className="min-h-screen bg-gray-50 flex items-center justify-center">
          <div className="text-center">
            ...
            <p className="mt-4 text-gray-600">Loading vegan products...</p>
          </div>
        </div>
        <Footer/>
      </div>
      ...
      <div>
        <div className="bg-green-50 py-12 text-center">
          <div className="max-w-4xl mx-auto px-4">
            ...
            <h1 className="text-4xl font-bold text-gray-900 mb-4">Vegan Products</h1>
            <p className="text-xl text-gray-600 mb-6">
              Discover our carefully curated selection of plant-based, cruelty-free products
            </p>
            <div className="bg-white rounded-lg p-4 inline-flex items-center gap-2">
              <Leaf size={20} className="text-green-500" />
              <span className="text-green-600 font-semibold">
                {products.length} Vegan Products Available
              </span>
            </div>
          </div>
        </div>
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <nav className="mb-8">
              <ol className="flex items-center space-x-2 text-sm text-gray-600">
                ...
                <ChevronDown size={16} className="rotate-[-90deg] mx-2" />
                <li className="text-green-600 font-medium">Vegan Products</li>
              </ol>
            </nav>
            ...
            <div className="mb-6">
              <div className="relative max-w-md">
                <input
                  ...
                  className="w-full pl-4 pr-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent"
                />
                <Leaf className="absolute right-3 top-1/2 transform -translate-y-1/2 text-green-400" size={20} />
              </div>
            </div>
              ...
              <div className={`${showFilters ? 'w-80' : 'w-0'} transition-all duration-300 flex-shrink-0 overflow-hidden`}>
                {showFilters && (
                  <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 h-fit sticky top-32">
                    ...
                    {/* Vegan-specific attributes */}
                    <FilterSection title="Vegan Attributes">
                      <div className="space-y-3">
                        ...
                        {availableFilters.hasKosher && (
                          <label className="flex items-center gap-3 cursor-pointer group">
                            <input
                              ...
                              className="w-4 h-4 text-green-500 border-gray-300 rounded focus:ring-green-500"
                            />
                            <span className="text-sm text-gray-700 group-hover:text-gray-900">Kosher Certified</span>
                          </label>
                        )}
                      </div>
                    </FilterSection>
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2 ml-4">
                      ...
                      <button
                        ...
                        className={`p-2 rounded-lg ${viewMode === 'list' ? 'text-green-600' : 'text-gray-400 hover:text-gray-600'}`}
                      >
                        <List size={20} />
                      </button>
                    </div>
...
// Vegan Product Card Component
function VeganProductCard({ product, viewMode, getImageUrl }) {
  ...
  if (viewMode === "list") {
    return (
      <Link href={detailLink}>
        <div className="rounded-lg border border-green-100 p-6 hover:shadow-lg transition-all cursor-pointer bg-white">
          <div className="flex gap-6">
            <div className="relative w-32 h-32 flex-shrink-0">
              <Image
                src={imageUrl}
                alt={product.name || 'Vegan product'}
                fill
            ...
            <div className="flex-1">
              <div className="flex items-start justify-between mb-3">
                <div>
                  <h3 className="text-xl font-semibold text-gray-900 mb-2 hover:text-green-600 transition">
                    {product.name || 'Vegan Product'}
                  </h3>
                  {brandName && (
  ...
  return (
    <Link href={detailLink}>
      <div className="rounded-lg shadow-sm overflow-hidden hover:shadow-md transition-all group cursor-pointer border border-green-100 bg-white">
