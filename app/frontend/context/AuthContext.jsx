...
export const AuthProvider = ({ children }) => {
  ...
  const login = async (email, password, rememberMe = false) => {
    try {
      const response = await fetch('https://devagroupon.onrender.com/api/auth/login', {
        method: 'POST',
        headers: {
  ...
  const isTokenValid = () => {
    ...
    const fetchblogs = async () => {
        try {
            const response = await axios.get("https://devagroupon.onrender.com/api/blog/getblog");
